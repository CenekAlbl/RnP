...
{
	if(nrhs<2||nlhs<5){
		mexErrMsgTxt("Minimum call: [R,w,C,t,f] = R7Pf(X,u)\n X: 3x6 matrix of 3D points \n u: 2x6 matrix of 2D projections\n");
		return;
	}
	const mwSize	*dims = mxGetDimensions(prhs[0]);
	if(dims[0]!=3 || dims[1]!=7){
		mexErrMsgTxt("First argument must be a 3x7 matrix of 3D points");
	}
	dims = mxGetDimensions(prhs[1]);
	if(dims[0]!=2 || dims[1]!=7){
		mexErrMsgTxt("Second argument must be a 2x7 matrix of 2D projections");
	}
	double r0 = 0;
	if(nrhs>2){
		r0= *mxGetPr(prhs[2]);
	...
	double *w_out, *t_out, *C_out, *v_out, *f_out;
