...
def r6pSingleLinpy(np.ndarray[double, ndim=2, mode="fortran"] X not None,
            np.ndarray[double, ndim=2, mode="fortran"] u not None,
            direction
            ):
    ...
    nres = r6pSingleLin(<double*> np.PyArray_DATA(X), <double*> np.PyArray_DATA(u), direction,0,1, <double*> np.PyArray_DATA(v), <double*> np.PyArray_DATA(w), <double*> np.PyArray_DATA(C), <double*> np.PyArray_DATA(t))

    return v[:,:nres], C[:,:nres], w[:,:nres], t[:,:nres]

def r6pDoubleLinpy(np.ndarray[double, ndim=2, mode="fortran"] X not None,
            np.ndarray[double, ndim=2, mode="fortran"] u not None,
            direction
            ):
    cdef np.ndarray[double, ndim=2, mode="fortran"] v = np.empty((3,64),order='fortran')
    ...
    nres = r6pDoubleLin(<double*> np.PyArray_DATA(X), <double*> np.PyArray_DATA(u), direction, <double*> np.PyArray_DATA(C), <double*> np.PyArray_DATA(t), <double*> np.PyArray_DATA(v), <double*> np.PyArray_DATA(w), 0)
